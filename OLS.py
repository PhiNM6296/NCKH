...
R1=pd.DataFrame({'SIZE':np.log(CEO_df['TotalAssets'].copy().dropna())})

#R1=pd.DataFrame({'SIZE':np.log(np.array(CEO_df['TotalAssets']))})

R2=pd.DataFrame({'ROE':CEO_df['ProfitAfterTax'].copy().dropna()/CEO_df['Equity'].copy().dropna() })

R3=pd.DataFrame({'ROA':CEO_df['ProfitAfterTax'].copy().dropna()/CEO_df['TotalAssets'].copy().dropna() })

R4=pd.DataFrame({'ROS':CEO_df['ProfitAfterTax'].copy().dropna()/CEO_df['Revenue_Net'].copy().dropna()})

R5=pd.DataFrame({'LEV':CEO_df['Debt'].copy().dropna()/CEO_df['TotalAssets'].copy().dropna() })

R6=pd.DataFrame({'FOR':CEO_df['FOR'].copy().dropna() })

R7=pd.DataFrame({'GOV':CEO_df['GOV'].copy().dropna() })

R8=pd.DataFrame({'CHAIR':CEO_df['CHAIR'].copy().dropna() })

R9=pd.DataFrame({'Duality':CEO_df['Duality'].copy().dropna() })

R10=pd.DataFrame({'CEO1':CEO_df['CEO1'].copy().dropna() })

R11=pd.DataFrame({'BSIZE':CEO_df['BSIZE'].copy().dropna() })
